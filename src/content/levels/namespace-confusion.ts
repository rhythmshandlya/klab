...
export const namespaceConfusion = {
  ...
  constraints: [
    { id: "edit-storefront", label: "Only edit storefront.yaml" },
    { id: "keep-namespaces", label: "checkout stays in the shop namespace" },
  ],
  files: [{ path: "storefront.yaml", language: "yaml", initialValue: STOREFRONT_YAML }],
  readonlyFiles: [
    { path: "checkout.yaml", language: "yaml", value: CHECKOUT_YAML },
    { path: "storefront-svc.yaml", language: "yaml", value: STOREFRONT_SVC_YAML },
  ],
  initialManifests: [NAMESPACE_YAML, CHECKOUT_YAML, STOREFRONT_SVC_YAML],
  registeredImages: [
    {
      ref: "klab/api:1.0.0",
      description: "Storefront API — proxies GET / to UPSTREAM_URL and reports the result.",
    },
    {
      ref: "klab/web-app:1.0.0",
      description: "Checkout web server — /healthz 200, / 200.",
    },
  ],
  allowedCommands: [
    "kubectl get pods",
