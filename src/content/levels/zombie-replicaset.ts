...
export const zombieReplicaset = {
  ...
  constraints: [
    { id: "edit-legacy", label: "Only edit legacy-rs.yaml — the Deployment and Service are correct" },
    { id: "keep-capacity", label: "Keep the stable pods serving throughout" },
  ],
  files: [{ path: "legacy-rs.yaml", language: "yaml", initialValue: LEGACY_RS_YAML }],
  readonlyFiles: [
    { path: "deployment.yaml", language: "yaml", value: DEPLOYMENT_YAML },
    { path: "service.yaml", language: "yaml", value: SERVICE_YAML },
  ],
  initialManifests: [DEPLOYMENT_YAML, SERVICE_YAML],
  registeredImages: [
    {
      ref: "klab/web-app:1.0.0",
      description: "Current build — /healthz 200, / 200.",
    },
    {
      ref: "klab/web-app:0.9.0",
      description: "Retired legacy build — /healthz still 200, but / answers 500.",
    },
  ],
  allowedCommands: [
    "kubectl get pods",
  ...
  evidenceRules: [
    {
      ...
      source: "network",
      trigger: { type: "probe", pathMatches: "^/$", status: 500 },
    },
    {
    ...
    },
    {
      ...
      source: "terminal",
      trigger: { type: "command", commandMatches: "get rs|get replicasets", outputMatches: "web-legacy" },
    },
    {
      id: "r-legacy-pod",
