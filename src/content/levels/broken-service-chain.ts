...
export const brokenServiceChain = {
  ...
  constraints: [
    { id: "edit-web-svc", label: "Only edit web-svc.yaml — every Deployment is correct" },
    { id: "keep-chain", label: "Keep the frontend → orders → web chain intact" },
  ],
  files: [{ path: "web-svc.yaml", language: "yaml", initialValue: WEB_SVC_YAML }],
  readonlyFiles: [
    { path: "web.yaml", language: "yaml", value: WEB_YAML },
    { path: "orders.yaml", language: "yaml", value: ORDERS_YAML },
    { path: "frontend.yaml", language: "yaml", value: FRONTEND_YAML },
  ],
  initialManifests: [WEB_YAML, ORDERS_YAML, FRONTEND_YAML],
  registeredImages: [
    {
      ref: "klab/api:1.0.0",
      description: "Proxy tier (frontend & orders) — calls UPSTREAM_URL, reports the result.",
    },
    {
      ref: "klab/web-app:1.0.0",
      description: "Web tier listening on 8080 — /healthz 200, / 200.",
    },
  ],
  allowedCommands: [
    "kubectl get pods",
  ...
  evidenceRules: [
    {
      ...
      source: "network",
      trigger: { type: "probe", pathMatches: "^/$", status: 502 },
    },
    {
      id: "r-web-dead",
      evidenceId: "web-dead",
    ...
    },
    {
      ...
      source: "terminal",
      trigger: { type: "command", commandMatches: "logs", outputMatches: "upstream call failed" },
    },
    {
      id: "r-targetport",
